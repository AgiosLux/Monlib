...
use Monlib\Controllers\Lists\{ListsMeta, ListsInspect};
...
class ListsList extends Response {
	...
	public function listAllLists(?string $privacy, ?int $offset = 0, ?int $limit = 50) {
		...
		$query			=	$this->orm->select($conditions, [
			'slug', 'title', 'total_access', 'total_downloads', 'list_file', 'added_in', 'updated_in'
		], $offset, $limit);
		
		foreach ($query as $key => $value) {
			$totalPdfs				=	new ListsInspect($this->username, $value['slug']);
			...
			$data[]					=	[
				...
				'total_documents'	=>	$totalPdfs->getListFileTotalLinksProfile($value['list_file']),
				'url'				=>	[
